...
aws s3 --endpoint-url https://s3.filebase.com cp s3://$S3_KEY_PATH /var/data/miner/swarm_key
...
if [ ! -z $UPLOAD_KEY ] && [ -f "/var/data/miner/swarm_key" ]; then
  echo "Uploading key"
  aws s3 --endpoint-url https://s3.filebase.com cp /var/data/miner/swarm_key s3://$S3_KEY_PATH
fi
